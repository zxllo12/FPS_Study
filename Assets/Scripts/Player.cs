...
public class Player : MonoBehaviour
{
    ...
    [SerializeField] Transform _cameraPoint; // 카메라가 따라갈 포인트
    ...
    private void Start()
    {
        ...
        // 카메라 설정
        _mainCamera.transform.SetParent(gameObject.transform); // 카메라를 플레이어의 자식으로 설정
        _mainCamera.transform.position = gameObject.transform.position + _offset; // 카메라 위치를 초기화

        CameraController cam = _mainCamera.GetComponent<CameraController>();
        cam.FollowTarget = _cameraPoint; // 카메라가 cameraPoint를 따라가도록 설정
